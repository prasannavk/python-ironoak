...
class HostSpatialsCalc:
    ...
    def _check_input(self, roi, frame):  # Check if input is ROI or point. If point, convert to ROI
        ...
        self.DELTA = 5  # Keep x,y within DELTA of the edge of the image
    ...
    def calc_spatials(self, depthFrame, roi, averaging_method=np.mean):
        ...
        centroid = {'x': int((xmax + xmin) / 2), 'y': int((ymax + ymin) / 2)}  # Get centroid of the ROI
        ...
        spatials = {'z': averageDepth, 'x': averageDepth * math.tan(angle_x), 'y': -averageDepth * math.tan(angle_y)}
